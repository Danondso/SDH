...
public class Map
{
	...
	private Rooms[][] createBlueprint(Tiles t, ItemPool items)
	{
		...
		for (int i = 0; i < MapSize; i++)
		{
			for (int j = 0; j < MapSize; j++)
			{
				...
				if (r == 1 && RoomItem == null) RoomItem = items.GetHealthPotion();

				if (i == 3 && j == 3) RoomItem = items.NextItem();

				else if (SpawnItem(i, j)) RoomItem = items.NextItem();

				if (RoomItem != null) blueprint[i][j] = new Rooms(t, player, i, j, RoomItem.Clone());
				else blueprint[i][j] = new Rooms(t, player, i, j, null);
				RoomItem = null;
				if (blueprint[i][j] == null) System.out.printf("What is this even\n");
	...
	public boolean MapUpdate()
	{
		...
		if (!room.getCreArray().isEmpty())
		{
			for (Creature cr : room.getCreArray())
			{
				if (cr.GetX() < 0) cr.SetX(0);

				if (cr.GetX() + cr.getImage().getHeight(null) > 32 * 16) cr.SetX(32 * 16 - cr.getImage().getHeight(null));

				if (cr.GetY() < 0) cr.SetY(0);

				if (cr.GetY() + cr.getImage().getWidth(null) > 32 * 16) cr.SetY(32 * 16 - cr.getImage().getWidth(null));
			}
